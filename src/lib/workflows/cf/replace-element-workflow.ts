 ...
 * `engine-registry.ts` flips `replace-element` to `'cloudflare'`. See
 ...
 */
function decideBatchOutcome(results: FrameResult[]): BatchOutcome {
 ...
 */
function shouldDowngradeVisionOnFailure(current: ElementVisionStatus): boolean {
 ...
 */
function rejectionReasonMessage(reason: unknown): string {
