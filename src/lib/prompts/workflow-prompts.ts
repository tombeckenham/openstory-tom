...
- ONE MOTION BEAT PER SCENE, ACHIEVABLE FROM A SINGLE STILL. Each scene needs motion an image-to-video model can produce from one frame: a subject's movement (a hand lifts the lid, fabric falls, steam curls, a smile breaks, a car surges forward) and/or a simple camera move (push-in, pull-out, pan, tilt, handheld drift, parallax, rack focus). Atmosphere and mood are welcome — let weather, light, and stillness saturate the frame — as long as one genuine beat of motion lives inside it. Never write a move that has to reveal a room, geometry, a location, or a subject not already in the frame; image-to-video warps instead of revealing, so if you imagine a "pull back to reveal…", cut it and frame the subject directly.
