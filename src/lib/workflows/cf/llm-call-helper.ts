/**
 * Cloudflare-Workflows port of `src/lib/workflows/llm-call-helper.ts`.
 *
 * Mirrors the QStash helper one-for-one — same step names, same control
 * flow, same side effects. The only differences are:
 *
 *   - Takes a `WorkflowStep` (from `cloudflare:workers`) instead of a
 *     QStash `WorkflowContext<TInput>`.
 *   - Uses `step.do` instead of `context.run`.
 *   - Throws `NonRetryableError` inside `step.do` bodies for unrecoverable
 *     errors so CF doesn't retry validation failures (the base class only
 *     re-wraps at the runImpl boundary).
 *
 * The QStash version stays as-is — both helpers coexist while workflows
 * migrate. See docs/investigations/cloudflare-workflows-poc.md.
 */
